...
import { 
  Plus, Search, ChevronDown, Mail, Bike,
...
export const Clientes = () => {
  ...
  const { clientes, agregarCliente, actualizarCliente, vehiculos } = useApp();

  const [busqueda, setBusqueda] = useState("");
  const [filtroEstado, setFiltroEstado] = useState("Todos los estados");
  const [showModal, setShowModal] = useState(false);
  const [editId, setEditId] = useState(null); // Cambiado a ID para mayor seguridad
  ...
  // Obtener las motos del cliente seleccionado en el detalle
  const motosDelClienteSeleccionado = detalleCliente 
    ? vehiculos.filter(v => v.cliente.toLowerCase() === detalleCliente.nombre.toLowerCase())
    : [];
    ...
    <div className="animate-in space-y-8 pb-10">
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-8 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.nombre} onChange={handleChange("nombre")} placeholder="Ej: Carlos Rodríguez"
              ...
              <div>
                ...
                <input value={form.email} onChange={handleChange("email")} placeholder="Ej: correo@email.com" type="email"
              ...
              <div>
                ...
                <input value={form.telefono} onChange={handleChange("telefono")} placeholder="Ej: +57 312 345 6789"
              ...
              <div>
                ...
                <input value={form.direccion} onChange={handleChange("direccion")} placeholder="Ej: Calle 50 #25-30, Bogotá"
            ...
            <div className="flex gap-3 mt-7">
              ...
              <button onClick={handleSubmit} className="flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl font-bold text-sm transition-all">
                {editId !== null ? "Guardar Cambios" : "Registrar Cliente"}
              </button>
            </div>
          </div>
        </div>
      ...
      {detalleCliente && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-8 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex justify-between bg-zinc-900/60 border border-zinc-800 rounded-2xl p-4 mb-5">
              {/* Cálculo dinámico también en la vista de detalle */}
              <div><p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Vehículos</p><p className="text-xl font-black text-white">{motosDelClienteSeleccionado.length}</p></div>
            ...
            {/* SECCIÓN DE MOTOS VINCULADAS EN REAL-TIME */}
            <div className="mb-6 pt-4 border-t border-zinc-900">
              <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Bike size={14}/> Motos en Taller</p>
              {motosDelClienteSeleccionado.length === 0 ? (
                <p className="text-xs text-zinc-600 italic pl-1">Sin motos registradas actualmente</p>
              ) : (
                <div className="space-y-1.5 max-h-28 overflow-y-auto pr-1 custom-scrollbar">
                  {motosDelClienteSeleccionado.map(moto => (
                    <div key={moto.id} className="flex justify-between items-center bg-zinc-900/80 px-3 py-2 rounded-xl border border-zinc-800/60">
                      <span className="text-xs text-zinc-300 font-semibold">{moto.marca} {moto.modelo}</span>
                      <span className="text-[10px] font-mono font-bold bg-zinc-950 text-purple-400 px-2 py-0.5 rounded border border-purple-500/10 uppercase">{moto.placa}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative group">
          ...
          <input type="text" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar por nombre, email o teléfono..." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {clientesFiltrados.map((cliente) => {
          // Filtrado en tiempo real de los vehículos que coinciden con este cliente
          const motosPropias = vehiculos.filter(v => v.cliente.toLowerCase() === cliente.nombre.toLowerCase());

          return (
            <div key={cliente.id || cliente.nombre} className="bg-zinc-950 border border-zinc-800 rounded-[2rem] p-6 hover:border-purple-500/30 hover:shadow-xl hover:shadow-purple-900/10 transition-all group">
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1 pt-1">
                  <h3 className="text-white font-bold text-lg leading-tight truncate">{cliente.nombre}</h3>
                  <span className={`inline-block mt-1 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md ${cliente.estado === 'Activo' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-zinc-800 text-zinc-400 border border-zinc-700'}`}>
                    {cliente.estado}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center gap-3 text-sm text-zinc-400"><Mail size={16} className="text-zinc-500 shrink-0" /><span className="truncate">{cliente.email}</span></div>
                <div className="flex items-center gap-3 text-sm text-zinc-400"><Phone size={16} className="text-zinc-500 shrink-0" /><span>{cliente.telefono}</span></div>
                <div className="flex items-center gap-3 text-sm text-zinc-400"><MapPin size={16} className="text-zinc-500 shrink-0" /><span className="truncate">{cliente.direccion}</span></div>
              </div>

              {/* CAJA DE ESTADÍSTICAS MOTO-CONECTADAS */}
              <div className="bg-zinc-900/50 rounded-2xl p-4 flex items-center justify-between border border-zinc-800/50 mb-4">
                <div>
                  ...
                  {/* El contador ahora lee el largo del filtro dinámico */}
                  <p className="text-xl font-black text-white">{motosPropias.length}</p>
                </div>
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-zinc-800/80">
                <button onClick={() => setDetalleCliente(cliente)} className="flex-1 flex items-center justify-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white py-2.5 rounded-xl text-sm font-bold transition-colors">
                  <Eye size={16} /> Ver Detalles
                </button>
                <button onClick={() => openModal(cliente)} className="p-2.5 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-xl transition-colors" title="Editar cliente">
                  <Edit3 size={18} />
                </button>
              </div>
