...
const CATEGORIAS = ["Lubricantes", "Filtros", "TransmisiÃ³n", "ElÃ©ctrico", "Frenos", "Llantas", "CarrocerÃ­a", "Otros"];
...
export const Inventario = () => {
  const [items, setItems] = useState([
    ...
    { nombre: "Cadena DID 520", marca: "DID Racing", codigo: "CAD-003", categoria: "TransmisiÃ³n", stock: 2, minStock: 5, precio: "$120.000", ubicacion: "Estante C-3", color: "from-emerald-500 to-teal-600" },
    { nombre: "BujÃ­a NGK Iridium", marca: "NGK", codigo: "BUJ-004", categoria: "ElÃ©ctrico", stock: 45, minStock: 15, precio: "$35.000", ubicacion: "Estante D-1", color: "from-orange-500 to-red-600" },
  ]);
  ...
  const itemsFiltrados = items.filter(item => {
    ...
    const catOk = filtroCategoria === "Todas las categorÃ­as" || item.categoria === filtroCategoria;
  ...
  const validate = () => {
    ...
    if (!form.codigo.trim()) e.codigo = "El cÃ³digo es obligatorio.";
    if (!form.stock || isNaN(Number(form.stock)) || Number(form.stock) < 0) e.stock = "Ingresa un stock vÃ¡lido (â‰¥0).";
    if (!form.minStock || isNaN(Number(form.minStock)) || Number(form.minStock) < 0) e.minStock = "Ingresa un mÃ­nimo vÃ¡lido (â‰¥0).";
    if (!form.precio.trim()) e.precio = "El precio es obligatorio.";
    if (!form.ubicacion.trim()) e.ubicacion = "La ubicaciÃ³n es obligatoria.";
    return e;
  };
  ...
  const handleSubmit = () => {
    ...
    if (editIndex !== null) {
      setItems(prev => prev.map((it, i) => i === editIndex ? { ...it, nombre: form.nombre, marca: form.marca, codigo: form.codigo, categoria: form.categoria, stock: Number(form.stock), minStock: Number(form.minStock), precio: form.precio, ubicacion: form.ubicacion } : it));
      showToast("Repuesto actualizado correctamente.");
    } else {
      setItems(prev => [...prev, { nombre: form.nombre, marca: form.marca, codigo: form.codigo, categoria: form.categoria, stock: Number(form.stock), minStock: Number(form.minStock), precio: form.precio, ubicacion: form.ubicacion, color: COLORES[prev.length % COLORES.length] }]);
  ...
  const handleReabastecer = () => {
    ...
    if (!cantidadReabastecer || isNaN(cant) || cant <= 0) { setErrorReabastecer("Ingresa una cantidad vÃ¡lida mayor a 0."); return; }
  ...
  return (
    <div className="animate-in space-y-8 pb-10">
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-8 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                { field: "codigo", label: "CÃ³digo *", placeholder: "Ej: ACE-001" },
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">CategorÃ­a *</label>
              ...
              {[
                ...
                { field: "minStock", label: "Stock MÃ­nimo *", placeholder: "Ej: 10" },
                { field: "precio", label: "Precio Unit. *", placeholder: "Ej: $15.000" },
                { field: "ubicacion", label: "UbicaciÃ³n *", placeholder: "Ej: Estante A-1" },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-5 flex flex-col justify-center shadow-sm">
          <div className="flex items-center gap-2 text-zinc-500 mb-2"><Tag size={16} /><p className="text-sm font-medium">CategorÃ­as</p></div>
      ...
      {stockBajoItems.length > 0 && (
        <div className="bg-orange-500/10 border border-orange-500/20 rounded-3xl p-6 flex gap-4 items-start shadow-lg shadow-orange-500/5">
          ...
          <div>
            <h3 className="text-orange-500 font-bold text-lg mb-1">AtenciÃ³n: Stock Bajo ({stockBajoItems.length} items)</h3>
      ...
      {itemsFiltrados.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-zinc-600">
          ...
          <p className="text-sm mt-1">Intenta con otro tÃ©rmino de bÃºsqueda o filtro.</p>
        </div>
      )}
